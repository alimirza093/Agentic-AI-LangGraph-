...
def scriptwriter_node(state: State) -> dict:
...
def translator_node(state: State) -> dict:
    """Stage 3: Translates the script into natural flowing Roman Urdu."""
    print("\n--- [Stage 3] Executing Roman Urdu Translator Node ---")
    
    prompt = (
        "You are an expert content localizer for the Pakistan market. Take the following script "
        "and convert it into natural, flowing 'Roman Urdu'. Do not simply translate it sentence-by-sentence "
        ...
        "Return only the final Roman Urdu text.\n\n"
        f"Script:\n{state['script_text']}"
    )
